...
let
  ...
  ediff-tool = pkgs.stdenv.mkDerivation {
    name = "ediff-tool";
    src = "${inputs.emacstool.outPath}/ediff.sh";
    dontUnpack = true;
    installPhase = ''
      mkdir -p $out/bin
      cp $src $out/bin/
  ...
  emacsPackage =
    ...
    (emacs-pkg.override {
      ...
      LSP_USE_PLISTS = "true"; # NOTE: also set as sesssion variable (in case required for 'doom build' command)
    });
  ...
  e-wrapper = pkgs.writeShellScriptBin "e" (builtins.readFile ./e);

  doom-corfu = "${inputs.doom-corfu.outPath}/modules/completion/corfu";
